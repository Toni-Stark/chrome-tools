...
function startGetUrlReferer(arr, index) {
  ...
  if (!current?.run && serviceId) {
    current = { url: arr[index], run: true };
    chrome.tabs.update(serviceId, {
      url: arr[index]
    });
...
chrome.tabs.onUpdated.addListener(function (tabId, info, tab) {
  ...
  if (!tabList[currentIndex]) {
    ...
    if (tabList[currentIndex][tabList[currentIndex].length - 1] !== tab.url) {
      tabList[currentIndex].push(tab.url);
    }
  }
  if (tab.status === 'complete') {
    tabTimer = setTimeout(() => {
      ...
      if (dataList.length === Object.keys(tabList).length) {
        clearData();
      }
      // });
    }, 2000);
